...
const DesktopIcons = () => {
    ...
    if (action === "scroll") {
      ...
      if (iconItem && iconItem.url) {
        if (iconItem.url.includes("your-resume-url-here")) {
          alert(
            `Opening ${iconItem.label}... (Note: Resume link is not added yet.)`
          );
        } else {
  ...
  const iconsList = [
    ...
    },
    {
      ...
      action: "url",
      url: "www.linkedin.com/in/pramit-bhattarai",
    },
    {
      id: "github",
      label: "GitHub",
  ...
  return (
    <>
      ...
      <div
        ref={containerRef}
        className={`fixed left-6 top-1/2 -translate-y-1/2 z-30 w-[96px] py-5 bg-[#7b9ebd] border-[3px] border-[#002c91] rounded-xl flex flex-col gap-5.5 items-center select-none font-sans transition-transform duration-300 ${isOpen ? "translate-x-0" : "-translate-x-[150px]"
          }`}
        style={{
          boxShadow: isOpen
      ...
      >
        ...
        {iconsList.map((item) => {
          ...
          return (
            <div
              ...
              className={`flex flex-col items-center justify-center p-1.5 cursor-pointer rounded border transition-all duration-100 ${isSelected
            ...
            >
              {item.icon}
              <span
                className={`text-[11px] mt-1 text-center font-medium leading-tight select-none px-1 rounded-sm ${isSelected ? "bg-[#0b61e2] text-white" : "text-shadow-desktop text-white"
                  }`}
                style={{
        ...
        className={`flex flex-col items-center justify-center p-1.5 cursor-pointer rounded border transition-all duration-100 ${selectedIcon === "recycle-bin"
          ? "bg-[#0b61e2]/25 border-dashed border-[#0b61e2] text-white"
          : "border-transparent text-slate-100 hover:bg-white/10"
          }`}
      ...
      >
        ...
        <span
          className={`text-[11px] mt-1 text-center font-medium leading-tight select-none px-1 rounded-sm ${selectedIcon === "recycle-bin"
